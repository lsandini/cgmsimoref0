...
async function getEntries(baseUrl, apiSecret, count = 144) {
  try {
    ...
    const agent = isHttps ? new (require('https')).Agent({ rejectUnauthorized: false }) : null;
    ...
    if (data.length > 0) {
      ...
      logger.debug(`First few entries timestamps: ${data.slice(0, 5).map(e => e.dateString || new Date(e.date).toISOString()).join(', ')}`);
    }
...
async function getTreatments(baseUrl, apiSecret, count = 288) {
  try {
    logger.debug(`Fetching ${count} treatments from Nightscout using downloads()`);
    
    const data = await downloads(baseUrl, apiSecret);
    
    logger.info(`TREATMENTS COMPARISON: downloads() returned ${data.treatments.length} treatments vs requested ${count} treatments`);
    logger.debug(`Received ${data.treatments.length} treatments from Nightscout`);
    if (data.treatments.length > 0) {
      logger.debug(`Sample treatment: ${JSON.stringify(data.treatments[0])}`);
      logger.debug(`First few treatments timestamps: ${data.treatments.slice(0, 5).map(t => t.created_at || new Date(t.date || t.mills).toISOString()).join(', ')}`);
    }
...
/**
 * Upload treatments to Nightscout (custom implementation)
 * @param {string} baseUrl - Nightscout URL
 * @param {string} apiSecret - API secret for Nightscout
 * @param {Array} treatments - Treatments to upload
 * @returns {Promise<Object>} - Response from Nightscout
 */
async function uploadTreatments(baseUrl, apiSecret, treatments) {
  try {
    ...
    const endpoint = `${url}/api/v1/treatments`;
    const response = await fetch(endpoint, {
      method: 'POST',
      headers,
      agent,
...
async function handleDeviceStatus(baseUrl, apiSecret, deviceStatuses) {
  try {
    ...
    if (Array.isArray(deviceStatuses)) {
      ...
      logger.debug(`Successfully uploaded ${deviceStatuses.length} device statuses to Nightscout`);
...
function createNightscoutClient(config) {
  ...
  return {
    ...
    getTreatments: (count) => getTreatments(config.url, config.apiSecret, count),
    uploadTreatments: (treatments) => uploadTreatments(config.url, config.apiSecret, treatments),
    getProfile: () => getProfile(config.url, config.apiSecret),
    uploadDeviceStatus: (deviceStatuses) => handleDeviceStatus(config.url, config.apiSecret, deviceStatuses)
  };
}
