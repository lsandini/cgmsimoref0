...
class Loop {
  ...
  async runCycle() {
    ...
    try {
      ...
      console.log('About to upload device status');
      console.log('Current IOB:', JSON.stringify(iobData[0], null, 2));
  ...
  async calculateIOB() {
    ...
    try {
      ...
      // Find the most recent bolus with proper error handling
      ...
      const enhancedIOB = {
        ...
        // Additional insulin tracking
        pumpBasalIOB: currentIOB.pumpBasalIOB || 
        (currentIOB.basaliob !== 0 ? currentIOB.basaliob : 0),
      ...
      // Add last bolus information if available with proper timestamp handling
      if (bolusEntries.length > 0) {
        const lastBolus = bolusEntries[0];
        const bolusTimestamp = lastBolus.timestamp || lastBolus.date;
        
        // Ensure timestamp is a valid string or convert from number if needed
        const bolusTime = typeof bolusTimestamp === 'number' 
          ? new Date(bolusTimestamp).toISOString() 
          : bolusTimestamp;
        
        enhancedIOB.lastBolusTime = new Date(bolusTime).getTime();
        enhancedIOB.lastBolusAmount = lastBolus.amount;
        
        // Log for debugging
        console.log(`Last bolus: ${lastBolus.amount}U at ${bolusTime}`);
        console.log(`Minutes since last bolus: ${Math.round((Date.now() - enhancedIOB.lastBolusTime) / 60000)}`);
      } else {
        // If no bolus found, set a default value to avoid NaN
        enhancedIOB.lastBolusTime = 0;
        enhancedIOB.lastBolusAmount = 0;
      ...
      const fallbackIOB = [{
        ...
        lastBolusTime: 0, // Set default to avoid NaN
        lastBolusAmount: 0, // Set default
        iobWithZeroTemp: {
          iob: 0,
          activity: 0,
          basaliob: 0,
  ...
  async determineBasal() {
    ...
    try {
      ...
      // Debug logging for IOB data to check last bolus time
      const iobData = JSON.parse(fs.readFileSync(path.join(MONITOR_DIR, 'iob.json')));
      console.log('Last bolus time check:', {
        lastBolusTime: iobData[0].lastBolusTime,
        currentTime: Date.now(),
        minutesSince: iobData[0].lastBolusTime ? 
          Math.round((Date.now() - iobData[0].lastBolusTime) / 60000) : 
          'No previous bolus'
      });
      ...
      // Log full determine-basal output for debugging SMB issues
      const recommendations = JSON.parse(result);
      console.log('Full determine-basal output:', JSON.stringify(recommendations, null, 2));
  ...
  async enactTreatments(recommendations) {
    ...
    // If we have insulin required, try to deliver an SMB
if (recommendations.insulinReq > 0) {
  // Calculate SMB size - this is how oref0 typically does it
  const smbAmount = Math.min(
    recommendations.insulinReq,
    0.3  // Maximum SMB size in units (adjust based on your preferences)
  );
  
  if (smbAmount > 0) {
    // Add bolus to pump history
    const now = new Date().toISOString();
    const newBolus = {
      type: 'Bolus',
      _type: 'Bolus',
    ...
    // Upload bolus to Nightscout
    const nsBolus = {
      eventType: 'Bolus',
      insulin: smbAmount,
      created_at: now,
      enteredBy: 'cgmsimoref0',
      notes: 'SMB from oref0'
    };
    
    await this.nightscout.uploadTreatments([nsBolus]);
    
    console.log(`SMB delivered: ${smbAmount}U`);
  }
}
    ...
    try {
      ...
      // Debug: Log IOB data before creating deviceStatus
      console.log('Raw IOB Data Before DeviceStatus:', JSON.stringify(iobData[0], null, 2));
      ...
      const deviceStatus = {
        ...
        openaps: {
          iob: {
            ...
            pumpBasalIOB: iobData[0].pumpBasalIOB || 
            (iobData[0].basaliob !== 0 ? iobData[0].basaliob : 0),
            iobWithZeroTemp: {
          ...
          suggested: {
            ...
            IOB: recommendations.IOB || 
            (iobData[0].iob !== 0 ? iobData[0].iob : 
             (iobData[0].basaliob !== 0 ? iobData[0].basaliob : 0)),
