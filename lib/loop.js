...
class Loop {
  ...
  async enactTreatments(recommendations) {
    ...
    // No actual pump, just record the recommendation as if it were enacted
    ...
    if (recommendations.units > 0) {
      const pumpHistoryPath = path.join(MONITOR_DIR, 'pumphistory-24h-zoned.json');
      const pumpHistory = fs.existsSync(pumpHistoryPath) ? 
        JSON.parse(fs.readFileSync(pumpHistoryPath)) : [];
      
      const bolus = {
        type: 'Bolus',
        timestamp: new Date().toISOString(),
        amount: recommendations.units,
        programmed: recommendations.units,
        unabsorbed: 0,
        duration: 0
      };
      
      pumpHistory.unshift(bolus); // Add to beginning of array
      
      // Limit history to 24 hours to avoid excessive growth
      const twentyFourHoursAgo = new Date();
      twentyFourHoursAgo.setHours(twentyFourHoursAgo.getHours() - 24);
      
      const filteredHistory = pumpHistory.filter(entry => {
        return new Date(entry.timestamp) >= twentyFourHoursAgo;
      });
    ...
    try {
      ...
      console.error('Error uploading treatments to Nightscout:', error);
    }
