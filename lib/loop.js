  ...
  async enactTreatments(recommendations) {
    ...
    try {
      ...
      if (recommendations.predBGs) {
        // Ensure all prediction arrays exist, even if empty
        const predBGs = {
          IOB: recommendations.predBGs.IOB || [],
          ZT: recommendations.predBGs.ZT || [],
          COB: recommendations.predBGs.COB || [],
          UAM: recommendations.predBGs.UAM || []
        };

        // Truncate prediction arrays to a consistent length (e.g., 24 points)
        Object.keys(predBGs).forEach(key => {
          if (predBGs[key].length > 24) {
            predBGs[key] = predBGs[key].slice(0, 24);
          }
        });

        deviceStatus.openaps.enacted.predBGs = predBGs;
      }
