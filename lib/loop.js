...
class Loop {
  ...
  async calculateIOB() {
    console.log('Calculating detailed IOB...');
    try {
      process.chdir(path.join(__dirname, '..'));
      
      // Read the current pump history
      ...
      // Find the latest temp basal
      const tempBasals = pumpHistory.filter(entry => entry._type === 'TempBasal');
      if (tempBasals.length > 0) {
        console.log('Latest temp basal in history:', JSON.stringify(tempBasals[0]));
      } else {
        console.log('No temp basals found in pump history!');
      }
